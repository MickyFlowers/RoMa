...
class RegressionMatcher(nn.Module):
    ...
    def match(
        ...
        *args,
        ...
        elif isinstance(im_A_path, Image.Image):
            im_A, im_B = im_A_path, im_B_path
        elif isinstance(im_A_path, np.ndarray):
            im_A, im_B = Image.fromarray(im_A_path).convert("RGB"), Image.fromarray(
                im_B_path
            ).convert("RGB")
        ...
        with torch.no_grad():
            ...
            if self.upsample_preds:
                ...
                if isinstance(im_A_path, (str, os.PathLike)):
                    ...
                    im_A, im_B = test_transform(
                        (
                            Image.fromarray(im_A_path).convert("RGB"),
                            Image.fromarray(im_B_path).convert("RGB"),
                        )
                    )
