...
class TinyRoMa(nn.Module):
    ...
    def pos_embed(self, corr_volume: torch.Tensor):
        ...
        if not self.training and not self.exact_softmax:
            ...
            P_lowres = torch.cat(
                (
                    cv[:, ::down, ::down].reshape(B, H1 * W1 // down**2, H0, W0),
                    best_match[:, None],
                ),
                dim=1,
            ).softmax(dim=1)
    ...
    def corr_volume(self, feat0, feat1):
        ...
        corr_volume = torch.einsum("bci,bcj->bji", feat0, feat1).reshape(
            B, H1, W1, H0, W0
        ) / math.sqrt(
            C
        )  # 16*16*16
    ...
    def match(self, im0, im1, *args, batched=True):
        ...
        if isinstance(im0, (str, Path)):
            ...
            batched = False
            device = self.device
            pil_img0 = Image.fromarray(im0).convert("RGB")
            pil_img1 = Image.fromarray(im1).convert("RGB")
            im0 = ToTensor()(pil_img0)[None].to(device)
            im1 = ToTensor()(pil_img1)[None].to(device)
        ...
        flow = (
            F.interpolate(
                corresps[4]["flow"], size=(H0, W0), mode="bilinear", align_corners=False
            )
            .permute(0, 2, 3, 1)
            .reshape(B, H0, W0, 2)
        )
    ...
    def forward(self, batch):
        ...
        B, C, H0, W0 = im0.shape
        B, C, H1, W1 = im1.shape
        to_normalized = torch.tensor((2 / W1, 2 / H1, 1)).to(im0.device)[
            None, :, None, None
        ]
        ...
        coarse_matches = torch.cat(
            (coarse_warp, torch.zeros_like(coarse_warp[:, -1:])), dim=1
        )
        feats_x1_c_warped = F.grid_sample(
            feats_x1_c,
            coarse_matches.permute(0, 2, 3, 1)[..., :2],
            mode="bilinear",
            align_corners=False,
        )
        coarse_matches_delta = self.coarse_matcher(
            torch.cat((feats_x0_c, feats_x1_c_warped, coarse_warp), dim=1)
        )
        coarse_matches = coarse_matches + coarse_matches_delta * to_normalized
        corresps[8] = {
            "flow": coarse_matches[:, :2],
            "certainty": coarse_matches[:, 2:],
        }
        coarse_matches_up = F.interpolate(
            coarse_matches,
            size=feats_x0_f.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        ...
        feats_x1_f_warped = F.grid_sample(
            feats_x1_f,
            coarse_matches_up_detach.permute(0, 2, 3, 1)[..., :2],
            mode="bilinear",
            align_corners=False,
        )
        fine_matches_delta = self.fine_matcher(
            torch.cat(
                (feats_x0_f, feats_x1_f_warped, coarse_matches_up_detach[:, :2]), dim=1
            )
        )
